...
content = content.replace(
  /const nextIndex\s*=\s*Math\.min\(routeIndex \+ ROUTE_JUMP, route\.length - 1\);\s*const \[nextLng, nextLat\] = route\[nextIndex\];\s*const \[prevLng, prevLat\] = route\[routeIndex\];\s*const distKm\s*=\s*haversineKm\(prevLat, prevLng, nextLat, nextLng\);\s*const speed\s*=\s*Math\.min\(110, Math\.max\(20, \(distKm \/ \(TICK_MS \/ 3_600_000\)\) \* 8\)\);/,
