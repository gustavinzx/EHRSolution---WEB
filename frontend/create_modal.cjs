...
export default function FuelUnlockModal() {
  ...
  const handleUnlock = async () => {
    setAuthorizing(true);
    try {
      const res = await fetch(\`${import.meta.env.VITE_API_URL || 'http://localhost:3001/api'}/fleet/\${awaitingTruck.id}/force-fueling\`, {
        method: 'POST'
      });
      if (res.ok) {
...
`;
fs.writeFileSync("src/components/FuelUnlockModal.jsx", content);
console.log("FuelUnlockModal created!");
